...
public class PongPacket_Tests
{
    private static PongPacket CreateRandomPongPacket()
        => new PongPacket();

    [Fact]
    public void Write_Parse_Roundtrips_AllProperties()
    {
        var packet = CreateRandomPongPacket();

        var buffer = new byte[packet.Length()];
        packet.Write(buffer);

        var parsed = Assert.IsType<PongPacket>(Packet.Parse(buffer));
    }

    [Fact]
    public void Parse_ExistingByteArray_WriteBack_EqualsOriginal()
    {
        var originalPacket = CreateRandomPongPacket();
        var originalBytes = originalPacket.ToByteArray();

        var parsed = Assert.IsType<PongPacket>(Packet.Parse(originalBytes));
