...
public class YamabikoPeer_Tests : IAsyncLifetime, IDisposable
{
    ...
    [InlineData(0.7)]
    [InlineData(0.6)]
    public async Task StreamSendReceive(double successChance)
    {
        var projectId = Guid.NewGuid();
        ...
        var data1 = Enumerable.Range(0, 4096).Select(x => (byte)x).ToArray();
        int offset1 = 0;
        while(offset1 < data1.Length)
        {
            var len = Math.Min(Random.Shared.Next(2048, 3072), data1.Length - offset1);
            await stream1.WriteAsync(data1.AsMemory(offset1, len));
            offset1 += len;
        }

        var data1Read = new byte[data1.Length];
        await stream2.ReadExactlyAsync(data1Read);
        Assert.Equal(data1, data1Read);

        var data2 = RandomNumberGenerator.GetBytes(Random.Shared.Next(4096, 8192));
        int offset2 = 0;
        while (offset2 < data2.Length)
        {
            var len = Math.Min(Random.Shared.Next(2048, 3072), data2.Length - offset2);
            await stream2.WriteAsync(data2.AsMemory(offset2, len));
            offset2 += len;
        }
        ...
        public override async Task SendAsync(IPEndPoint endpoint, ReadOnlyMemory<byte> buffer, CancellationToken ct)
        {
            if (Random.Shared.NextDouble() <= _successChance)
            {
                if (endpoint.Equals(ServerTransport.Endpoint))
                    await _serverTransport.Channel.Writer.WriteAsync((Endpoint, buffer), ct);
                else if (_clients.TryGetValue(endpoint, out var client))
                    await client.Channel.Writer.WriteAsync((Endpoint, buffer), ct);
            }
        }
